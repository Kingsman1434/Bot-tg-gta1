...
async def date_from(update: Update, context: CallbackContext) -> int:
    ...
    if not is_valid_date(date_from):
        await update.message.reply_text('Неверный формат даты. Введите дату начала (в формате ДД.ММ.ГГГГ):')
        return DATE_FROM
...
async def date_to(update: Update, context: CallbackContext) -> int:
    ...
    if not is_valid_date(date_to):
        await update.message.reply_text('Неверный формат даты. Введите дату окончания (в формате ДД.ММ.ГГГГ):')
        return DATE_TO
...
async def check_online(update: Update, context: CallbackContext, nick: str, min_per: str, max_per: str) -> str:
    ...
    try:
        ...
        min_per_formatted = None
    try:
        max_per_formatted = datetime.strptime(max_per, '%d.%m.%Y').strftime('%Y-%m-%d')
    except ValueError:
        max_per_formatted = None

    if min_per_formatted and max_per_formatted:
        ...
        return f"Неверный формат даты для никнейма {nick}. Проверьте введенные даты."
